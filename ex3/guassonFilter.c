...
void createGaussianKernel(int radius, double sigma, double **kernel, double *sum) {
    ...
    double sumReduction = 0.0; // reduction variable

    *kernel = (double *)malloc(kernelWidth * kernelWidth * sizeof(double));

    #pragma omp parallel for reduction(+:sumReduction) collapse(2) // "collapse(2)" parallelize the two nested loops
    for (int x = -radius; x <= radius; x++) {
        for (int y = -radius; y <= radius; y++) {
            ...
            sumReduction += kernelValue; // add to reduction variable
        }
    }
...
Image *createBlurredImage(int radius, Image *image) {
    ...
    // we put the x loops inside the block loop for better cache performance
    #pragma omp parallel for schedule(static, 16)
    for (int y = radius; y < height - radius; y++) {
        for (int block = radius; block < width - radius; block += 16) {
            for (int x = block; x < block + 16 && x < width - radius; x++) {
                double redValue = 0.0;
